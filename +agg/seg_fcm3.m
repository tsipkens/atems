...
%== STEP 3: Perform c-means segmentation ==================================%
disp('Performing c-means clustering...');
[~,grade_fcm] = fcm(double(reshape(...
    feature_set,[numel(img),length(feature_set(1,1,:))])),2);
disp('Complete.');
disp(' ');

grade_fcm1 = reshape(grade_fcm(1,:),size(img));
bw0 = grade_fcm1>0.5;
[~,ind_max] = max([mean(img(bw0)),mean(img(~bw0))]);
if ind_max==1; grade_fcm1 = reshape(grade_fcm(2,:),size(img)); end

img_fcm = grade_fcm1>0.3;
