...
%     close (gcf);
    ...
    %== Step 2: Automatic/semi-automatic Aggsregate detection =============%
%     imgs(ii).Binary = pcm.Aggs_detection(imgs(ii),pixsize,moreAggss,minparticlesize,coeffs);
%     
%     imgs(ii).Sobel   = edge(imgs(ii).Binary,'sobel'); % Sobel edge detection
%     SE              = strel('disk',1);
%     imgs(ii).DilatedEdge = imdilate(imgs(ii).Sobel,SE); % morphological dilation
%     
%     clear img.Edge SE2
%     imgs(ii).Imposed = imimposemin(imgs(ii).Cropped, imgs(ii).DilatedEdge);
%     
%     [imgs(ii).Binary,imgs(ii).dilatedEdge,imgs(ii).Imposed] =  ... 
%         thresholding_ui.detectAggsregate(imgs(ii));

% 
% 
%     % Run Sobel Edge Detection
%     imgs(ii).Sobel = edge(imgs(ii).Binary,'sobel');
%     
%     % Run Morphological Dilation
%     SE = strel('disk',1);
%     imgs(ii).DilatedEdge = imdilate(imgs(ii).Sobel,SE);
%     
%     %-- Generate imposed image -------------------------------------------%
%     imgs(ii).Imposed = imimposemin(imgs(ii).Cropped, imgs(ii).DilatedEdge);
    ...
    %-- Lopp for each Aggsregate on the image under investigation ---------%
        
        Data = struct; % initialize data structure for current Aggsregate
        Data.method = 'pcm';
        
        
        %-- Step 3-2: Prepare an image of the isolated Aggsregate ---------%
%         Data.Image = zeros(size(imgs(ii).Binary));
%         Data.Image(CC.PixelIdxList{1,nAggs}) = 1;
        ...
        % be considered
        Image_edge = edge(Aggs(ll).binary,'sobel');
        
        % Dilated Edge Image
        % Use dilation to strengthen the Aggsregate's outline
        SE = strel('disk',1);
        img_dilated = imdilate(Image_edge,SE);
        img_final_imposed = imimposemin(Aggs(ll).image, img_dilated);
        figure
        hold on
        ...
        Skel = bwmorph(Aggs(ll).binary,'thin',Inf);
        [skeletonY, skeletonX] = find(Skel);
        
        %-- 3-3-2: Calculate the distances between skeleton pixels and other pixels
        clear row col
        [row, col] = find (Aggs(ll).binary);
        
        
        Data.Pixels = nnz(Aggs(ll).binary); % total number of pixels in Aggsregate
        ...
        %-- 3-4: Computing Aggsregate dimentions/parameters ---------------%
        % Projected area of the Aggsregate
        Data.area = Data.Pixels*pixsize^2;
        
        % Projected area equivalent diameter of the Aggsregate
        ...
        for kk = 1:1:Data.Pixels
            if  (Aggs(ll).binary(row(kk)-1,col(kk))==0) || ...
                (Aggs(ll).binary(row(kk),col(kk)+1)==0) || ...
                (Aggs(ll).binary(row(kk),col(kk)-1)==0) || ...
                (Aggs(ll).binary(row(kk)+1,col(kk))==0)
        ...
        path     = sprintf('PCF_%d_%d_new',nAggs,ll);
        ...
        %-- Clear variables ----------------------------------------------%
        clear Aggs.Image Image_edge FinalImposedImage Skel skeletonY ...
            skeletonX Aggs.Pixels Distance_mat Radius PCF PCF_smoothed ...
            Denamonator row col
        
        %== Step 4: Save results =========================================%
        %   Format output and autobackup data ----------------------------%
        Aggs(ll).pcm = Data; % copy Dp data structure into img_data
        ...
        extracted_data(1) = round(Aggs.Area,2);
        extracted_data(2) = round(Aggs.Perimeter,2);
        extracted_data(3) = round(Aggs.L,2);
        extracted_data(4) = round(Aggs.W,2);
        extracted_data(5) = round(Aggs.Aspectratio,2);
        extracted_data(6) = round(Aggs.Rg,2);
        extracted_data(7) = round(Aggs.da,2);
        extracted_data(8) = round(2*Aggs.RpSimple,2); % dp from simple PCM
        extracted_data(9) = round(2*Aggs.RpGeneralized,2); % dp from generalized PCM
